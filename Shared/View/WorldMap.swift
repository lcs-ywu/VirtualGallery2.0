...
struct WorldMap: View {
    ...
    var body: some View {
        ...
        ZStack{
            ...
            Map(coordinateRegion: $region, annotationItems: allMuseums) { museum in
                
                
                MapAnnotation(coordinate: CLLocationCoordinate2D(latitude: museum.latitude, longitude: museum.longitude)) {
                    
                    if museum.name != "LCS Gallery" {
                        NavigationLink(destination: MuseumDetailView(museum: museum)) {
                            
                            Image(museum.name).resizable().cornerRadius(10).scaledToFit().frame(width: 80, height: 80).shadow(radius: 3)
                            
                        }
                    } else {
                        NavigationLink(destination: LCSArtCommunityView()) {
                            
                            Image("LCS").resizable().cornerRadius(10).scaledToFit().frame(width: 80, height: 80).shadow(radius: 3)
                        }
